...
def main():
    ...
    for category in scraper.CATEGORIES:
        df = scraper.fetch_category_single_season(category, season)
        name_col = "player.name" if "player.name" in df.columns else None
        if not name_col:
            print(f"[{category}] לא נמצאה עמודת שם שחקן. עמודות קיימות: {list(df.columns)}")
            continue

        matches = df[df[name_col].astype(str).str.upper().str.contains(query_text, na=False)]
        print(f"[{category}] סה\"כ {len(df)} שורות. ", end="")
        if matches.empty:
            print(f"'{query_text}' לא נמצא.")
        else:
            cols_to_show = [c for c in ["player.code", "player.name", "player.team.code", "gamesPlayed"] if c in df.columns]
