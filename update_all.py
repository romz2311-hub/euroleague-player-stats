...
רשמיים + מדינות לשחקנים חדשים) ובונה מחדש את הדשבורד. לא נוגע בעונות
ישנות (אלה לא משתנות) - רק בעונה הנוכחית, כדי שזה יהיה מהיר ולא יעמיס
מיותר על ה-API.
...
def main():
    ...
    print(f"--- שלב 1/4: סטטיסטיקת שחקנים לעונה {current_season} ---")
    scraper.store_full_history(start_season=current_season, end_season=current_season)

    print("\n--- שלב 2/4: סגלים רשמיים (עונה נוכחית) ---")
    fetch_rosters.main()

    print("\n--- שלב 3/4: מדינות לשחקנים חדשים ---")
    fetch_countries.main()

    print("\n--- שלב 4/4: בניית דשבורד ---")
