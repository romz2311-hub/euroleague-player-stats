...
## סטטיסטיקה לפי משחק (box-score) - שחקנים עם מעט הופעות

הדוחות העונתיים הרגילים (`main.py`) לא כוללים כל שחקן שהופיע אי פעם - יש
כנראה סף מינימלי של דקות/משחקים כדי להיכלל שם. שחקנים עם מעט מאוד הופעות
(למשל שחקני הרכב מקומיים) פשוט לא יופיעו שם בכלל, גם אם באמת שיחקו.

כדי לתפוס גם אותם, יש שכבה נפרדת שמושכת סטטיסטיקה **לפי משחק בודד**
(box-score) - כל שחקן רשום מקבל שורה בכל משחק, גם אם ישב על הספסל כל
המשחק:
...
ברירת מחדל: רק העונה הנוכחית. זה איטי (עד 450 קודי משחק עם השהיה בין
בקשות ונסיון חוזר אם נחסמים) - ריצה ראשונה יכולה לקחת 20-30 דקות, אבל
משחק שכבר נשמר לא נשלף שוב, אז ריצות הבאות (כמו מתוך `update_all.py`)
הרבה יותר מהירות.

`build_dashboard.py` כבר משלב אוטומטית שחקנים שקיימים רק במקור הזה (לא
בדוחות העונתיים) לתוך פאנל הסינון האינטראקטיבי.
...
ההיסטוריה - זה מהיר), סגלים רשמיים, מדינות לשחקנים חדשים, box-score
למשחקים חדשים, ובניית הדשבורד מחדש:
